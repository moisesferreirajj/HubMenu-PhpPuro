...
<body>
  ...
  <nav class="navbar navbar-expand-lg navbar-dark sticky-top">
    ...
    <!-- EXEMPLO DE COMO PUXAR CORES DO ESTABELECIMENTO
    <p style="background-color: <= htmlspecialchars($estabelecimento[0]->cor1) ?>;">
      < htmlspecialchars($estabelecimento[0]->endereco) ?>
    </p>
    -->
...
<div class="row row-cols-2 row-cols-md-2 row-cols-lg-4 g-4" id="produtos-lista">
  <?php foreach ($produtos as $produto): 
    ...
    // Atribuição automática das classes com base no ID
    if ($catId === 9) {
    ...
    fetch(`/api/produtos/procurar/${estabelecimentoId}?query=${encodeURIComponent(query)}`)
        ...
        .then(data => {
            if (data.status === 'success') {
                //LIMPA O CONTAINER ANTES DE ADICIONAR OS NOVOS PRODUTOS
                produtosContainer.innerHTML = '';

                //SE NAO ACHAR NENHUM PRODUTO, MOSTRA MENSAGEM
                if (data.produtos.length === 0) {
                    produtosContainer.innerHTML = '<p>Nenhum produto encontrado.</p>';
                    return;
                }

                //ADICIONA OS PRODUTOS NO CONTAINER
            ...
            //SE TIVER ERRO MOSTRE MENSAGEM
            else {
                produtosContainer.innerHTML = '<p>Erro ao buscar produtos.</p>';
            }
        })
        //CATCH PARA TRATAR DE ERRO DE REDE OU DE BUSCAR PRODUTOS
        .catch(err => {
            produtosContainer.innerHTML = '<p>Erro ao buscar produtos.</p>';
            console.error(err);
        });
